...
int FCFS_experiment::random_int(int min, int max) {
	random_device dev;
	mt19937 seed(dev());
...
FCFS_experiment::FCFS_experiment() {
	system("cls");
...
void FCFS_experiment::sort_process() {
	for (int i = 0; i < n - 1; i++)
		for (int j = i + 1; j < n; j++)
...
void FCFS_experiment::sort_result() {
	for (int i = 0; i < n - 1; i++)
		for (int j = i + 1; j < n; j++)
