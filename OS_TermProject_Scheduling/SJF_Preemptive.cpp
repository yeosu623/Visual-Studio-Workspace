...
int SJF_Preemptive::random_int(int min, int max) {
	random_device dev;
	mt19937 seed(dev());
...
void SJF_Preemptive::sort_process() {
	for (int i = 0; i < n - 1; i++)
		for (int j = i + 1; j < n; j++)
...
void SJF_Preemptive::sort_result() {
	for (int i = 0; i < n - 1; i++)
		for (int j = i + 1; j < n; j++)
